...
- `ops.Select` retains candidates through selectors such as `selectors.TopK`.
- `ops.QueryTarget` is the target-call boundary.
- `ops.Evaluate` writes evaluation results through `evaluators.Evaluator`.
- `ops.AddFeedback` turns observations into attacker context.
- `ops.StopWhen` consumes evaluations through `conditions.Condition`.
